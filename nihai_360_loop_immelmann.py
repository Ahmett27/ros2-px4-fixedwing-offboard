...
class FixedWingPositionControl(Node):
                ...
                if key.lower() == 'k':
                    ...
                    if self.current_pose is not None and self.current_pose.z > 150.0:
                        ...
                        if self.current_mode == "OFFBOARD":
                            ...
                            self.start_maneuver('roll', 5.0)  # 'roll' tipi manevra, 5 saniye sürecek
                        else:
                    ...
                    if self.current_pose is not None and self.current_pose.z > 150.0:
                        if self.current_mode == "OFFBOARD":
                            ...
                            # Tepeyi hızlı aşması için 3.0 saniye gibi biraz daha kısa/sıkı bir süre veriyoruz
    ...
    def start_immelmann(self):
        ...
        # Senin de dediğin gibi uçağın tepeye çıkması için tam 3.0 saniye veriyoruz.
        # 3 saniye dolduğunda 'immelmann_phase2' fonksiyonunu otomatik tetikleyecek.
    ...
    def immelmann_phase2(self):
        ...
        # Faz 1'in zamanlayıcısını iptal ediyoruz (arkada çöp bırakmamak için)
        self.timer_immelmann_phase2.cancel()
        
        # 1.5 saniye boyunca tono atıp uçağı düzelttikten sonra manevrayı tamamen bitir.
    ...
    def publish_control_commands(self):
        ...
        if self.is_maneuvering:
            ...
            if self.maneuver_type == 'roll':
                msg.body_rate.x = 1.7  # Yatay takla (Roll) hızı
                msg.body_rate.y = 0.08  # Burnun düşmemesi için hafif destek
                msg.body_rate.z = 0.2  
                msg.thrust = 0.8        # Tono atarken hızı korumak için %80 gaz
                

            elif self.maneuver_type == 'loop':
                msg.body_rate.x = 0.0
                # Uçağın yerçekimine yenilmeden tepe noktasını (apex) aşıp ters dönmesi için 
                # dönüş hızını -1.5'ten -2.2'ye çıkarıyoruz. (Daha dar ve hızlı bir çember)
                msg.body_rate.y = -1.7 
                msg.body_rate.z = 0.0
                msg.thrust = 1.0        # Tepeyi tırmanmak için tam gaz şart.



            elif self.maneuver_type == 'immelmann_pitch':
                msg.body_rate.x = 0.0
                msg.body_rate.y = -1.7  # Senin bulduğun tırmanma değeri
                msg.body_rate.z = 0.0
                msg.thrust = 1.0        # Yerçekimini yenmek için tam gaz
                
            elif self.maneuver_type == 'immelmann_roll':
                # Uçak ters dururken 180 derece (Yarım Tono) dönüp düzelmesi lazım.
                # 180 derece matematiksel olarak pi (3.14) radyandır.
                # Faz 2'ye 1.5 saniye süre verdiğimiz için hızı 2.1 rad/s veriyoruz ki (2.1 * 1.5 = ~3.14) tam düzelsin.
                msg.body_rate.x = 1.7   
                msg.body_rate.y = -1.7   # Tırmanmayı kes
                msg.body_rate.z = 0.2
                msg.thrust = 1.0        # Düz uçuş itkisine geri dön
                ...
                # Faz 2'ye 1.5 saniye süre verdiğimiz için hızı 2.1 rad/s veriyoruz ki (2.1 * 1.5 = ~3.14) tam düzelsin.
                msg.body_rate.x = 1.7   
                msg.body_rate.y = 0.0  # Tırmanmayı kes
                msg.body_rate.z = 0.2
                msg.thrust = 1.0        # Düz uçuş itkisine geri dön
