...
class FixedWingPositionControl(Node):
    ...
    def __init__(self, drone_id='uav1'):
        ...
        self.state_sub = self.create_subscription(
            State, 
            f'/{self.drone_id}/mavros/state', 
            self.state_callback, 
            10
        )
        
        # 2. DEĞİŞİKLİK: Manual control topic'i de aynı şekilde güncelleniyor
        self.rc_pub = self.create_publisher(
            ManualControl, 
            f'/{self.drone_id}/mavros/manual_control/send', 
            10
        )

        
        self.rc_pub = self.create_publisher(ManualControl, f'/{self.drone_id}/mavros/manual_control/send', 10)
        ...
        self.pos_pub = self.create_publisher(PoseStamped, f'/{self.drone_id}/mavros/setpoint_position/local', qos_profile)

        self.arming_client = self.create_client(CommandBool, f'/{self.drone_id}/mavros/cmd/arming')
        self.mode_client = self.create_client(SetMode, f'/{self.drone_id}/mavros/set_mode')
        self.param_client = self.create_client(ParamSet, f'/{self.drone_id}/mavros/param/set') 
...
def main(args=None):
    ...
    node = FixedWingPositionControl(drone_id='uav1')
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
